...
pub const Connection = struct {
    ...
    pub fn init(allocator: *Allocator, environment: *Environment) ReturnError!Connection {
    ...
    pub fn browseConnect(self: *Connection, partial_connection_string: []const u8) !?[]const u8 {
        ...
        var out_string_buffer = try self.allocator.alloc(u8, 100);
        errdefer self.allocator.free(out_string_buffer);
        ...
        run_loop: while (true) {
            ...
            switch (@intToEnum(SqlReturn, result)) {
                .Success, .SuccessWithInfo => {
                    ...
                    self.connected = true;
                    self.allocator.free(out_string_buffer);
                    return null;
                },
                .NeedsData => {
                ...
                .InvalidHandle => @panic("Connection.browseConnect passed invalid handle"),
                    ...
                    for (errors) |e| {
                        if (e == .StringRightTrunc) {
                            ...
                            // to store the result.
                            out_string_buffer = try self.allocator.realloc(out_string_buffer, @intCast(usize, out_string_len) + 1);
                            continue :run_loop;
                        }
                    }
                    return error.Error;
    ...
    pub fn getAllEnabledFunctions(self: *Connection) ![]odbc.FunctionId {
        var result_buffer: [c.SQL_API_ODBC3_ALL_FUNCTIONS_SIZE]c.SQLUSMALLINT = undefined;
        var result_list = std.ArrayList(odbc.FunctionId).init(self.allocator);
    ...
    pub fn nativeSql(self: *Connection, sql_statement: []const u8) ![]const u8 {
        ...
        // close
        var out_statement_buffer = self.allocator.alloc(u8, sql_statement.len);
        errdefer self.allocator.free(out_statement_buffer);
                ...
                .InvalidHandle => @panic("Connection.nativeSql passed invalid handle"),
                    ...
                    for (errors) |e| {
                        if (e == .StringRightTrunc) {
                            ...
                            out_statement_buffer = try self.allocator.realloc(out_statement_buffer, @intCast(usize, out_statement_len) + 1);
                            continue :run_loop;
                        }
                    }
    ...
    pub fn getInfo(self: *Connection, comptime info_type: InformationType) !InformationTypeValue {
        var result_buffer = try self.allocator.alloc(u8, 200);
        errdefer self.allocator.free(result_buffer);
            ...
            switch (@intToEnum(SqlReturn, result)) {
                .Success, .SuccessWithInfo => {
                    ...
                    if (!value.isStringType()) {
                        self.allocator.free(result_buffer);
                    }
                    return value;
                },
                .InvalidHandle => @panic("Connection.getInfo passed invalid handle"),
                    ...
                    for (errors) |e| {
                        if (e == .StringRightTrunc) {
                            result_buffer = try self.allocator.realloc(result_buffer, @intCast(usize, result_string_length));
                            continue :run_loop;
                        }
                    }
    ...
    pub fn getAttribute(self: *Connection, comptime attribute: Attribute) !?AttributeValue {
        var value = try self.allocator.alloc(u8, 100);
    ...
    pub fn getDiagnosticRecords(self: *Connection) ![]odbc_error.DiagnosticRecord {
        return try odbc_error.getDiagnosticRecords(self.allocator, odbc.HandleType.Connection, self.handle);
    }
};